...
from ..matching.similarity import SimilarityMatcher
...
class GapAnalyzer:
    ...
    def __init__(
        ...
        similarity_calculator: Optional[SimilarityMatcher] = None,
    ):
        """
        ...
        self.similarity_calculator = similarity_calculator or SimilarityMatcher()
    ...
    def analyze_skills_gap(
        self, resume_text: str, job_text: str, industry: str = "software"
    ) -> Dict[str, Any]:
        """
        ...
        match_percentage = (
            len(matching_skills) / len(job_skill_names) * 100 if job_skill_names else 0
        )
            ...
            if skill_name in missing_skills:
                ...
                if isinstance(skill, str):
                    missing_skill_details.append(
                        {"name": skill, "category": "Uncategorized", "confidence": 1.0}
                    )
                else:
        ...
        categorized_missing = self.skills_analyzer.classify_skills(
            missing_skill_details
        )
    ...
    def analyze_experience_gap(
        self, resume_experience: str, job_requirements: str
    ) -> Dict[str, Any]:
        """
        ...
        experience_gap = (
            required_years - resume_years if required_years > resume_years else 0
        )

        # Determine experience level from resume
        resume_experience_level = self.skills_analyzer.detect_experience_level(
            resume_experience
        )
        ...
        level_compatibility = self._calculate_level_compatibility(
            resume_experience_level, job_seniority
        )
        ...
        matching_experiences = self._match_experience_phrases(
            resume_experience, key_job_phrases
        )

        # Non-matching phrases represent experience gaps
        experience_gap_phrases = [
            phrase
            for phrase in key_job_phrases
    ...
    def analyze_education_match(
        self, resume_education: str, job_requirements: str
    ) -> Dict[str, Any]:
        """
        ...
        meets_requirement = self._check_degree_requirement(
            highest_resume_degree, minimum_required_degree
        )
    ...
    def get_comprehensive_gap_analysis(
        ...
        education_analysis = self.analyze_education_match(
            resume_education, job_requirements
        )
    ...
    def _calculate_level_compatibility(
        self, resume_levels: Dict[str, float], job_level: str
    ) -> float:
    ...
    def _extract_key_experience_phrases(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if not re.search(
                r"\b(?:experience|background|skill|proficiency|knowledge)\b",
                sentence.lower(),
            ):
                continue
    ...
    def _match_experience_phrases(
        self, resume_text: str, job_phrases: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        for phrase in job_phrases:
            ...
            similarity = self.similarity_calculator.semantic_similarity(
                phrase, resume_text
            )
            ...
            if similarity > 0.6:  # Threshold determined empirically
                matches.append(
                    {"phrase": phrase, "similarity": similarity, "is_match": True}
                )
            else:
                matches.append(
                    {"phrase": phrase, "similarity": similarity, "is_match": False}
                )
    ...
    def _check_degree_requirement(
        self, resume_degree: Dict[str, Any], required_degree: Dict[str, Any]
    ) -> bool:
    ...
    def _check_field_match(
        self, resume_fields: List[str], required_fields: List[str]
    ) -> Dict[str, Any]:
        ...
        missing_fields = [
            field for field in required_fields if field not in resume_fields
        ]
    ...
    def _calculate_education_match_score(
        self, meets_requirement: bool, field_match: Dict[str, Any]
    ) -> float:
        """Calculate education match score"""
    ...
    def _calculate_overall_match(
        ...
        experience_score = 1.0 - min(
            1.0, experience_analysis["experience_gap_years"] / 5
        )
    ...
    def _identify_improvement_areas(
            ...
            improvement_areas.append(
                {
                    ...
                    "details": [
                        skill["name"]
                        for skill in skills_analysis["critical_missing_skills"]
                    ],
                }
            )
            ...
            improvement_areas.append(
                {
                    ...
                    "details": [
                        f"Need {experience_analysis['required_years']} years, have {experience_analysis['resume_years']} years"
                    ],
                }
            )
            ...
            improvement_areas.append(
                {
                    ...
                    "details": [
                        f"Need {education_analysis['minimum_required_degree']['name']} degree or higher"
                    ],
                }
            )
        ...
        # Check for field of study match
        if (
            not education_analysis["field_match"]["is_match"]
            and education_analysis["field_match"]["missing_fields"]
        ):
            improvement_areas.append(
                {
                    "category": "education",
