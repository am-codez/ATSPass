...
class Recommender:
    ...
    def get_context_aware_suggestions(
        ...
        skills_suggestions = self._generate_skills_suggestions(
            gap_analysis["skills_gap_analysis"]
        )
        suggestions.extend(skills_suggestions)

        # Add experience-based suggestions
        experience_suggestions = self._generate_experience_suggestions(
            gap_analysis["experience_gap_analysis"]
        )
        ...
        education_suggestions = self._generate_education_suggestions(
            gap_analysis["education_match_analysis"]
        )
    ...
    def _generate_experience_suggestions(
        self, experience_gap: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
            ...
            suggestions.append(
                {
                    ...
                    "details": {
                        ...
                        "resume_levels": experience_gap.get(
        ...
        for phrase in experience_gap.get("experience_gap_areas", [])[
            :3
        ]:  # Limit to top 3
            suggestions.append(
                {
                    ...
                    "details": {"missing_experience": phrase},
                }
            )

        return suggestions

    def _generate_education_suggestions(
        self, education_match: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if not education_match.get("meets_requirement", True):
            min_degree = education_match.get("minimum_required_degree", {}).get(
                "name", ""
            )
            if min_degree:
                suggestions.append(
                    {
                        ...
                        "details": {
                            ...
                            "current_degree": education_match.get(
            ...
            suggestions.append(
                {
                    ...
                    "details": {"missing_sections": list(missing_sections)},
                }
            )
    ...
    def get_industry_specific_recommendations(
        self,
        resume: Dict[str, Any],
        job: Dict[str, Any],
        ...
        if self.use_llm:
            recommendations = self._enhance_industry_recommendations_with_llm(
                recommendations, resume, job, industry
            )
                ...
                for i, suggestion in enumerate(suggestions[:5]):
                    if i < len(enhanced_suggestions):
                        ...
                        if "description" in enhanced_suggestions[i]:
                            suggestions[i]["description"] = enhanced_suggestions[i][
                                "description"
                            ]
    ...
    def _enhance_industry_recommendations_with_llm(
        self,
                    ...
                    if i < len(enhanced_recommendations):
                        ...
                        if "description" in enhanced_recommendations[i]:
                            recommendations[i]["description"] = (
                                enhanced_recommendations[i]["description"]
                            )
                        if "title" in enhanced_recommendations[i]:
                            recommendations[i]["title"] = enhanced_recommendations[i][
                                "title"
                            ]
    ...
    def _enhance_ats_tips_with_llm(
        self, ats_tips: List[Dict[str, Any]], resume: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            data = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a helpful resume optimization assistant. Respond only with JSON.",
                    },
