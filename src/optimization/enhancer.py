...
class ContentEnhancer:
    ...
    def _integrate_keywords_with_llm(self, 
        ...
        try:
            ...
            # Create prompt for LLM
            prompt = f"""
            Enhance this text by naturally integrating these keywords: {', '.join(keyword_texts)}
            
            Original text:
            "{text}"
            
            Rules:
            - Make the additions sound natural and contextual
            - Maintain the original meaning and tone
            - Don't add more than 20% additional words
            - Preserve structure and formatting
            
            Return ONLY the enhanced text with no additional explanation.
            """
    ...
    def _call_llm_api_text(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            data = {
                "model": "gpt-4o-mini",  # Or your preferred model
                "messages": [
                    {"role": "system", "content": "You are a helpful resume optimization assistant. Provide only the requested content with no explanations."},
                    {"role": "user", "content": prompt}
                ],
                "temperature": 0.7
            }
