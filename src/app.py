...
from src.optimization.recommender import ResumeRecommender
...
similarity_calculator = SimilarityCalculator(semantic_analyzer=semantic_analyzer)
gap_analyzer = GapAnalyzer(skills_analyzer=skills_analyzer, similarity_calculator=similarity_calculator)
resume_recommender = ResumeRecommender(gap_analyzer=gap_analyzer, skills_analyzer=skills_analyzer, use_llm=USE_LLM, api_key=API_KEY)
content_enhancer = ContentEnhancer(keyword_extractor=keyword_extractor, semantic_analyzer=semantic_analyzer, text_cleaner=text_cleaner, use_llm=USE_LLM, api_key=API_KEY)
...
def render_analysis_page():
    ...
    if st.button("Enhance Resume"):
        if not hasattr(st.session_state, 'resume_text') or not hasattr(st.session_state, 'job_text'):
            st.error("Resume or job description not found. Please go back to the input page.")
            return
        
        # Run enhancement
        enhancement_results = enhance_resume(st.session_state.resume_text, st.session_state.job_text)
...
def render_optimization_page():
    ...
    if not hasattr(st.session_state, 'enhancement_results'):
        st.warning("No enhancement results available. Please analyze your resume first.")
        return
...
def analyze_resume_job_match(resume_text: str, job_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        job_keywords = [{'text': kw, 'score': score} for kw, score in job_keywords_tuples]
        
        # Perform gap analysis
        gap_analysis = gap_analyzer.get_comprehensive_gap_analysis(
            {'sections': cleaned_resume},
            {'sections': cleaned_job, 'metadata': {'industry': 'software'}}  # Default to software industry
        )
        
        # Get recommendations
        recommendations = resume_recommender.get_context_aware_suggestions(
            {'sections': cleaned_resume},
            {'sections': cleaned_job, 'metadata': {'title': parsed_job.get('title', '')}}
        )
        ...
        # Extract gap metrics
        skills_match = gap_analysis.get('skills_gap_analysis', {}).get('match_percentage', 0)
...
def enhance_resume(resume_text: str, job_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        enhanced_bullets = content_enhancer.enhance_bullet_points(bullet_points, enhancement_context)
        ...
        for section, content in parsed_resume.get('sections', {}).items():
            ...
            if relevant_keywords:
                enhancement_result = content_enhancer.integrate_keywords(content, relevant_keywords)
        ...
        keyword_match_increase = len([b for b in enhanced_bullets if b.get('matched_keywords')]) * 2
        section_improvement = sections_improved * 5
        enhanced_score = min(100, original_score + keyword_match_increase + section_improvement)
        
        # Count enhanced bullets
        bullets_enhanced = len([b for b in enhanced_bullets if b.get('enhancement_method') != 'unchanged'])
        ...
        response = {
            ...
            'added_keywords': [kw for section in enhanced_sections.values() 
...
def filter_keywords_for_section(section: str, keywords: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Filter keywords based on relevance to the resume section"""
    section = section.lower()
    
    # Define section-specific keyword filters
    section_keywords = {
        'summary': ['experience', 'professional', 'expertise', 'background', 'skilled', 'years'],
        'experience': ['managed', 'developed', 'created', 'led', 'implemented', 'designed', 'built'],
        'skills': ['proficient', 'expert', 'knowledge', 'advanced', 'familiar', 'skilled'],
        'education': ['degree', 'university', 'college', 'bachelor', 'master', 'phd', 'certification'],
        'projects': ['developed', 'created', 'built', 'designed', 'implemented', 'collaborated']
    }
