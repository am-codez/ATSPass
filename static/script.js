...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function getEnhancements() {
        try {
            ...
            // Make the actual API call - no more hardcoded values
            ...
            enhanceButtons.forEach(button => {
                ...
                button.textContent = 'Enhance Resume';
            });
        } catch (error) {
            console.error('Error:', error);
    ...
    function updateEnhancementsUI(result) {
        ...
        if (enhancedScore && result.enhanced_score !== undefined) {
            ...
            if (enhancedScoreMeterFill) {
                ...
                enhancedScoreMeterFill.style.backgroundColor = getScoreColor(score);
            }
        }
    ...
    // Set enhanced score meter fill
    if (enhancedScoreMeterFill) {
        enhancedScoreMeterFill.style.width = '86.7%';
        enhancedScoreMeterFill.style.backgroundColor = getScoreColor(86.7);
    }
});

// Function to get color based on score
function getScoreColor(score) {
    if (score >= 75) return '#4CAF50';  // Green for good scores
    if (score >= 50) return '#FFA726';  // Orange for medium scores
